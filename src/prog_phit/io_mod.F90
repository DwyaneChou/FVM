module io_mod
  ...
  implicit none
    ...
    subroutine history_write_stat(stat,time_slot_num)
      ...
      real(r_kind), dimension(:,:,:), allocatable :: phit
      ...
      allocate(phit(ids:ide,jds:jde,ifs:ife))
      ...
      do iPatch = ifs,ife
        do j = jds,jde
          do i = ids,ide
            phit(i,j,iPatch) = stat%q(1,i,j,iPatch) / sqrtGC(i,j,iPatch)
            phi (i,j,iPatch) = stat%q(1,i,j,iPatch) / sqrtGC(i,j,iPatch) - zsC(i,j,iPatch) * gravity
            u   (i,j,iPatch) = stat%q(2,i,j,iPatch) / ( stat%q(1,i,j,iPatch) - sqrtGC(i,j,iPatch) * zsC(i,j,iPatch) * gravity )
            v   (i,j,iPatch) = stat%q(3,i,j,iPatch) / ( stat%q(1,i,j,iPatch) - sqrtGC(i,j,iPatch) * zsC(i,j,iPatch) * gravity )
