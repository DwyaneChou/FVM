module spatial_operators_mod
  ...
  real(r_kind), dimension(:,:,:,:), allocatable :: dphitdy    ! d phi_t / dy
    ...
    subroutine reconstruction(q,qL,qR,qB,qT,qQ,dqdx,dqdy)
      ...
      real(r_kind), dimension(maxRecCells) :: p
            ...
            do i = ids,ide
              do iStencil = 1,nStencil_all
                ...
                elseif(iStencil==nStencil1+1)then
                  do iCOS = 1,nStencil1
                    sigma(iCOS) = ( 1. + ( ( abs(beta1(1)-beta1(2)) + abs(beta1(1)-beta1(3)) + abs(beta1(1)-beta1(4)) + abs(beta1(2)-beta1(3)) + abs(beta1(2)-beta1(4)) + abs(beta1(3)-beta1(4)) ) / nStencil1 )**2 / ( beta1(iCOS) + eps ) ) / nStencil1
                  enddo
                  sigma_sum = sum(sigma)
              ...
              !tau = ( sum( abs( beta(nStencil) - beta(1:nStencil-1) ) ) / ( nStencil - 1. ) )**( 2 * nStencil - 1 )
